...
sealed class ThesisPlannerScreenUIEvent { // Menyimpan event-event UI yang terkait dengan layar ThesisPlanner
    ...
    data class OnSaveTaskClick(val taskThesis: TaskThesis) : ThesisPlannerScreenUIEvent() // Event saat tombol simpan tugas diklik
    data class OnTaskCompletedStatusChange(val taskThesis: TaskThesis, val isCompleted: Boolean) : ThesisPlannerScreenUIEvent() // Event saat status tugas selesai berubah
    data class OnDeleteTaskClick(val taskThesis: TaskThesis) : ThesisPlannerScreenUIEvent() // Event ketika tombol hapus tugas diklik
    ...
    data class OnTaskDeleteConfirm(val taskThesis: TaskThesis) : ThesisPlannerScreenUIEvent() // Event saat konfirmasi penghapusan tugas
